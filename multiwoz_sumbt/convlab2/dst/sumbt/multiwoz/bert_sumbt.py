...
from convlab2.dst.sumbt.woz_en.convert_to_glue_format import convert_to_glue_format as convert_to_glue_format_woz_en
from convlab2.dst.sumbt.woz_de.convert_to_glue_format import convert_to_glue_format as convert_to_glue_format_woz_de
...
from convlab2.util.woz_en.state import default_state as default_state_woz_en
from convlab2.util.woz_de.state import default_state as default_state_woz_de
...
from convlab2.dst.sumbt.woz_en.sumbt_utils import Processor as ProcessorWoZEn
from convlab2.dst.sumbt.woz_de.sumbt_utils import Processor as ProcessorWoZDe
...
woz_slot_list_en = ['informable-area', 'informable-food', 'informable-pricerange', 'request-address', 'request-area', 'request-food', 'request-name', 'request-phone', 'request-postcode', 'request-pricerange']
woz_slot_list_de = ['informable-essen', 'informable-gegend', 'informable-preisklasse', 'request-adresse', 'request-essen', 'request-gegend', 'request-name', 'request-postleitzahl', 'request-preisklasse', 'request-telefon']
...
class SUMBTTracker(DST):
    ...
    def __init__(self, model_file='https://convlab.blob.core.windows.net/convlab-2/sumbt.tar.gz', arg_path=os.path.join(SUMBT_PATH,'config.json'),eval_slots=multiwoz_slot_list_en):
        ...
        elif args.lang == 'woz-en':
            convert_to_glue_format = convert_to_glue_format_woz_en
            default_state = default_state_woz_en
            processor = ProcessorWoZEn(args)
            eval_slots = woz_slot_list_en
        elif args.lang == 'woz-de':
            convert_to_glue_format = convert_to_glue_format_woz_de
            default_state = default_state_woz_de
            processor = ProcessorWoZDe(args)
            eval_slots = woz_slot_list_de
        ...
        self.cached_res = {}
        convert_to_glue_format(args)
        if not os.path.isdir(args.output_dir):
            os.makedirs(os.path.join(args.output_dir))
        self.train_examples = processor.get_train_examples(args.tmp_data_dir, accumulation=False)
        self.dev_examples = processor.get_dev_examples(args.tmp_data_dir, accumulation=False)
        self.test_examples = processor.get_test_examples(args.tmp_data_dir, accumulation=False)
    ...
    def init_session(self):
        ...
        if not self.param_restored:
            ...
            elif os.path.isfile(os.path.join(self.args.output_dir, 'pytorch_model.bin')):
                print('loading weights from trained model')
                self.load_weights(model_path=os.path.join(self.args.output_dir, 'pytorch_model.bin'))
            else:
    ...
    def update(self, user_act=None):
        ...
        for state in pred_states:
            ...
            if self.args.lang == 'en' or self.args.lang == 'zh':
    ...
    def test(self, mode='dev', model_path=None):
        ...
        # if TARGET_SLOT == 'all':
        #     out_file_name += '_all'
        output_eval_file = os.path.join(self.args.output_dir, "%s.txt" % out_file_name)
        ...
        with open(os.path.join(self.args.output_dir, "%s.txt" % out_file_name), 'w') as f:
            f.write(
