...
def convert_to_glue_format(args):
    ...
    ### Read ontology file
    with open(os.path.join(args.data_dir, "ontology.json"), "r") as fp_ont:
    ...
    fp_train = open(os.path.join(args.tmp_data_dir, "train.tsv"), "w")
    fp_dev = open(os.path.join(args.tmp_data_dir, "dev.tsv"), "w")
    fp_test = open(os.path.join(args.tmp_data_dir, "test.tsv"), "w")
    ...
    for split_type, split_fp in zip(file_split, fp):
        ...
        zipfile_name = "{}.json".format(split_type)
        #zip_fp = zipfile.ZipFile(os.path.join(data_dir, zipfile_name))
        data = json.load(open(os.path.join(args.data_dir,zipfile_name)))
