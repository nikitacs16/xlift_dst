...
class Processor(DataProcessor):
    ...
    def __init__(self, config):
        ...
        fp_ontology = open(os.path.join(config.data_dir, "ontology_sumbt.json"), "r")
        ...
        for slot in ontology.keys():
            if "informable" in slot:
                ontology[slot].append("es ist egal")
        fp_ontology.close()
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, max_turn_length):
    ...
    for (ex_index, example) in enumerate(examples):
        if 'E' in example.guid:
            curr_id = example.guid.replace("-E","")
        else:
            curr_id = example.guid
        
        if max_turn < int(curr_id.split('-')[2]):
            max_turn = int(curr_id.split('-')[2])
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        curr_dialogue_idx = example.guid.replace("-E","").split('-')[1]
        curr_turn_idx = int(example.guid.replace("-E","").split('-')[2])
