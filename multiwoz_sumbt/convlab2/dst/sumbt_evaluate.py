...
woz_en_slot_list = ['food','area', 'pricerange']
...
if __name__ == '__main__':
    ...
    if dataset_name.startswith('MultiWOZ'):
        ...
        from convlab2.util.dataloader.dataset_dataloader import MultiWOZDataloader, WOZDataloader
        dataloader = AgentDSTDataloader(dataset_dataloader=MultiWOZDataloader(config_data['lang']=='zh'))
        #dataloader = AgentDSTDataloader(dataset_dataloader=WOZDataloader(True))
        #dataloader.load_data(data_key=data_key)[data_key]
        data = dataloader.load_data(data_key=data_key, data_dir=os.path.join('/fs/startiger0/nmoghe/code/ConvLab-2',config_data['data_dir']))[data_key] #change the path here to your directory
        ...
        if len(curr_sess) > 0:
            all_predictions[session_count] = copy.deepcopy(curr_sess)
        new_data = {'context': context, 'predictions':all_predictions}
        json.dump(new_data, open(sys.argv[3],'w'))
