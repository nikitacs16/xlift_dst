...
class WOZDataloader(DatasetDataloader):
    def __init__(self, lang):
        super(WOZDataloader, self).__init__()
        self.lang = lang
    ...
    def load_data(self,
        ...
        assert role in ['sys', 'usr', 'all']
        info_list = ['context', 'belief_state']
        info_list = list(filter(eval, ['utterance', 'dialog_act', 'context', 'context_dialog_act', 'belief_state',
                                       'last_opponent_utterance', 'last_self_utterance', 'session_id', 'span_info',
                                      'terminated', 'goal']))
        info_list = list(filter(eval, ['utterance', 'dialog_act', 'context', 'context_dialog_act', 'belief_state',
        ...
        print(info_list)
        if self.lang == 'woz-en':
            bs_en = {'informable':{'food': 'none', 'area': 'none', 'price range': 'none'},
                    'request':{'area':'none','address': 'none', 'food': 'none', 'name':'none','phone':'none', 'price range': 'none', 'postcode': 'none'}

                    }
        elif self.lang == 'woz-de':
            bs_de = {            "informable":{
        ...
        for data_key in data_key_list:
            ...
            for sess in data:
                cur_context = []
                for i, turn in enumerate(sess['dialogue']):
                    if len(turn['system_transcript']) > 0:
                       cur_context.append(turn['system_transcript'])
                    text = turn['transcript']
                    cur_context.append(text)
                    #cur_context.append(turn['system_transcript'])
                    self.data[data_key]['context'].append(cur_context[-context_window_size:])
                    if self.lang == 'woz-en':
                        belief_state = copy.deepcopy(bs_en)
                    elif self.lang == 'woz-de':
                        belief_state = copy.deepcopy(bs_de)
                    ...
                    for bs in turn['belief_state']:
                      for slots in bs["slots"]:
                          if slots[0]!='slot':
                              ...
                              belief_state["informable"][slots[0]] = slots[1]
                          if slots[0] == "slot":
                              belief_state["request"][str(slots[1])] = "yes"

                    self.data[data_key]['belief_state'].append(belief_state)

                    #cur_context.append(text)
        if ontology:
            ontology_path = os.path.join(data_dir, 'ontology_sumbt.json')
            self.data['ontology'] = json.load(open(ontology_path))
